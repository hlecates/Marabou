...
- For "optimizing" I repeatedly call get methods from the torch model (ie getDependencies(), getDependencies(), getInputSize(), getInputLowerBounds(), getInputUpperBounds(), getOutputIndex(), getOutputSize() etc)
    Is it better to cache these value in the constructor? 
    Is using the get methods significantly slower? Or is the separation of concern between the torchModel and CROWN Analysis better?

- Use torch::autograd in the TorchModel?
    Do not think this is needed for CROWN analysis, but will need it for alpha CROWN
    to prevent manual gradient computation
...
void CROWNAnalysis::computeCrownBackwardPropagation()
{
    ...
    log(Stringf("computeCrownBackwardPropagation() - Output index determined: %u", outputIndex));
    
    if ( !_nodes.exists(outputIndex) ) 
    {
        log(Stringf("computeCrownBackwardPropagation() - Warning: Output index %u not found in nodes. Skipping CROWN analysis.", outputIndex));
        return;
    }
    ...
    log("computeCrownBackwardPropagation() - Starting queue processing");
    ...
    while (!queue.empty())
    {
        ...
        node->boundBackward(currentLowerAlpha, currentUpperAlpha, inputIBPBounds, A_matrices, lbias, ubias);
        ...
        if (_torchModel->getDependenciesMap().exists(current))
        {
            for (unsigned i = 0; i < _torchModel->getDependencies(current).size() && i < A_matrices.size(); ++i)
            {
                ...
                // No additional multiplication needed - bounded nodes already computed A matrices
        ...
        // after propagating from `current` remove the A matrices and bias terms for memory management
    ...
    log("computeCrownBackwardPropagation() - Completed.");
}
...
void CROWNAnalysis::concretizeBounds()
{
    ...
    // if shapes don't align, fallback to IBP
    if (lA.defined() && lA.dim() >= 2) {
        int nodeDim = inputLower.size(0);
        int expectedNodeDim = lA.size(-1);
        if (nodeDim != expectedNodeDim) {
            log(Stringf("concretizeBounds() - Dim mismatch: input=%d, expected=%d; fallback to IBP", nodeDim, expectedNodeDim));
            if (_ibpBounds.exists(outputIndex)) {
                _concreteBounds[outputIndex] = _ibpBounds[outputIndex];
                _torchModel->setConcreteBounds(outputIndex, _ibpBounds[outputIndex]);
            }
            log("concretizeBounds() - Completed (fallback: dim mismatch)");
            return;
        }
    }
    ...
    if (concreteLower.defined() && concreteUpper.defined()) {
        ...
        log("concretizeBounds() - Concrete bounds undefined; fallback to IBP if available");
        if (_ibpBounds.exists(outputIndex)) {
            _concreteBounds[outputIndex] = _ibpBounds[outputIndex];
            _torchModel->setConcreteBounds(outputIndex, _ibpBounds[outputIndex]);
        }
